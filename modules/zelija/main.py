...
class Circle(Shape):
    def draw(self, surf, rect):
        pygame.draw.circle(surf, self.color, rect.center, max(10, self.size // 2 - 12))
...
class Cell:
    def __init__(self, r, c):
        self.rect = pygame.Rect(c * CELL_SIZE, r * CELL_SIZE, CELL_SIZE, CELL_SIZE)
        self.shape = None
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    app.selected_shape = Circle
                if event.key == pygame.K_2:
                    app.selected_shape = Square  # type: ignore
                if event.key == pygame.K_3:
                    app.selected_shape = Triangle  # type: ignore
                if event.key == pygame.K_4:
                    app.selected_shape = Star  # type: ignore

                if event.key == pygame.K_d:
                    app.delete_mode = not app.delete_mode
                ...
                if app.selected_cell and app.selected_cell.shape:
                    if event.key == pygame.K_q:
                        app.selected_cell.shape.rotate(-15)
                    if event.key == pygame.K_e:
                        app.selected_cell.shape.rotate(15)
                    if event.key in (pygame.K_PLUS, pygame.K_EQUALS):
                        app.selected_cell.shape.size = min(
                            CELL_SIZE, app.selected_cell.shape.size + 6
                        )
                    if event.key == pygame.K_MINUS:
                        app.selected_cell.shape.size = max(
                            20, app.selected_cell.shape.size - 6
                        )
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, col in enumerate(PALETTE):
                    rect = pygame.Rect(
                        GRID_WIDTH + 20 + (i % 3) * 50, 380 + (i // 3) * 50, 40, 40
                    )
                ...
                for row in app.cells:
                    for cell in row:
                        if cell.rect.collidepoint(mx, my):
                            if app.delete_mode:
                                cell.shape = None
                                if cell == app.selected_cell:
                                    app.selected_cell = None
                            else:
                                if cell.shape:
                                    app.selected_cell = cell  # type: ignore
                                else:
                                    cell.shape = app.selected_shape(
                                        app.selected_color, CELL_SIZE
                                    )
                                    app.selected_cell = cell  # type : ignore

        for row in app.cells:
            for cell in row:
                cell.draw(screen, cell == app.selected_cell)
